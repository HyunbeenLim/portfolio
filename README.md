...
따라서 조사자의 연구 목적에 따라 모형 추정시 제거하지 않고 사용해도 괜찮다고 생각합니다.

#### 평균비교

일반적 특성에 따른 항목 별 평균 비교를 진행하였습니다.

일반적 특성이 10가지에 그룹 또한 2 그룹 ~ 7 그룹으로 나누어지기 때문에 평균분석을 진행한 결과와 빈도분석 그래프 등의 양이 방대해 모든 내용을 첨부할 순 없으나 그룹 별 차이가 있다는 결과를 보인 것에 대해 언급하겠습니다.

그룹이 두 그룹인 경우 T-test 사용하였고 세 그룹 이상인 경우 ANOVA를 사용한 뒤 그룹 간 유의한 차이가 있다고 나온 변수들에 대해선 Scheffe 사후검증하였습니다.

1. 먼저 성별 변수는 남자와 여자 두 그룹이 존재하며 학습전이에 대해서만 유의확률이 0.11로 귀무가설을 기각하여 남자와 여자의 학습전이 점수의 분포에 차이가 있다는 결과를 얻었습니다.
2. 학년 변수는 1학년 ~ 4학년 총 네 그룹이 존재하며 학업적 자기효능감 변수에서 귀무가설을 기각하였습니다. 사후분석 결과 학업적 자기효능감에서 2학년과 4학년의 분포가 다른 것으로 확인되었고 4학년보다 2학년의 자기효능감이 높았습니다.
3. 평점평균 변수는 학점 별 5그룹이 존재하며 자기결정성 학습동기와 학습성과 변수에서 그룹 간 차이가 있다는 결과를 얻었습니다. 사후분석 결과 자기결정성 학습동기 변수에서 평점평균이 2.5-3.0인 그룹이 3.0-3.5인 그룹, 3.5-4.0인 그룹 두 그룹과의 차이가 있다는 것으로 나타났으며 2.5-3.0 그룹이 다른 두 그룹보다 자기결정성 학습동기가 높다는 결과를 얻었습니다. 하지만 학습성과 변수에 대한 사후분석 결과 유의한 차이가 없는 것으로 나타났으며 이에 대한 이유는 사후검정의 보수적인 정도가 너무 강해서 입니다. 특히 Scheffe의 방법이 보수적인 정도가 너무 높기 때문에 해당 방법을 통한 사후분석 시 자주 발생하는 문제입니다. 해석은 ‘분산분석을 통한 집단별 차이는 유의하게 있었으나 Scheffe 사후분석을 시행한 결과에서는 유의한 차이가 없었다.’라고 할 수 있겠습니다.
4. 입학동기는 7가지의 그룹이 존재하며 학습성과와 학습전이 변수에서 그룹 간 차이가 있는 것으로 확인되었습니다. 사후분석 결과 학습성과에선 적성에 맞아서 입학한 그룹과 주변의 권유로 입학한 경우 두 그룹이 차이가 있는 것으로 나타났으며 적성에 맞아서 입학한 그룹의 학습성과가 높았습니다. 학습전이에선 사후분석의 보수적인 경향으로 유의미한 차이가 있는 그룹이 없는 것으로 나타났습니다.
5. 
